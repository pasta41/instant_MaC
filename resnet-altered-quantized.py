...
y = Dense(121, activation='relu', name='combine-cats')(newmodel.layers[-1].output)
newmodel2 = Model(inputs=newmodel.inputs, outputs=y)
combine_weights = np.zeros((126,121),np.float32)
combine_weights[:121] = np.identity(121,np.float32)
combine_weights[121:126][:,120] = 1
newmodel2.layers[176].set_weights([combine_weights, newmodel2.layers[176].get_weights()[1]])


expected_class = 0
correct = 0
total = 0
for directory in sorted(os.listdir('dataset-resized-120/validation/')):
    for image in os.listdir(os.path.join('dataset-resized-120/validation',directory)):
        original = load_img(os.path.join('dataset-resized-120/validation',directory,image), target_size=(224,224))
